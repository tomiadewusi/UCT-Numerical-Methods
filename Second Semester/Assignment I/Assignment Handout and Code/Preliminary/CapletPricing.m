...
mu_Y =@(t1,t2,Yt1,rt1) Yt1 + A(t1,t2).*rt1 + 0.5.*sigma_Y(t1,t2).^2 - C(t1,t2);
%--------------------------------------------------------------------------
mat = zeros(1,19);
for ix = 1:19
    index = ix; % These will be the inputs for the function
    K = 0.05;  % These will be the inputs for the function
    n = 30000;
    %--------------------------------------------------------------------
    % These all need to be row vectors so that they are compatible with the
    ...
    sample1 = (1./Beta_t2).*max(((1./Bt1t2)-1)-K,0);
    sample2 = 1./Bt1t2;
    exact = exp(-C(t1,t2)+A(t1,t2).*mu_r(t0,t1,r0)...
        +0.5* (A(t1,t2)*sigma_r(0,index)).^2 );
    n0 = n*.1;
    helper = cov(sample1(1:n0),sample2(1:n0));
    alpha_opt = -helper(2,1)/helper(2,2);
    Price = mean(sample1 + alpha_opt.*(sample2-exact)); 
    mat(ix) = Price;
end
t= toc;
time_seconds = 1000*(t/19)
figure()
hold on
plot(1:19,mat,'k.')
plot(1:19,Caplets,'r-')
figure()
plot(1:19,mat-Caplets)

error = (mat-Caplets)*(mat-Caplets).'
