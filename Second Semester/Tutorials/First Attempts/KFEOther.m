...
k=2*mu/(sigma^2*(2-beta)*(exp(mu*(2-beta)*T)-1));
xx=k*X0^(2-beta)*exp(mu*(2-beta)*T);
w=@(X) k*X.^(2-beta);
closed=@(X) (2-beta)*k^(1/(2-beta))*(xx*w(X).^(1-2*beta)).^(1/(4-2*beta)) ...
.*exp(-xx-w(X)).*besseli(1/(2-beta),2*sqrt(xx*w(X)));
...
B = @(X) -0.5.*sigma.^2.*(X.^(beta)); 
C = @(X) mu - ((beta/2).*sigma.*X.^(beta/2 -1)).^2 ...
    - (sigma.*X.^(beta/2)).*(beta/2).*(beta/2 -1).*sigma.*X^(beta/2 -2) ; 
