...
Vinf = @(S,t) S-K.*ones(size(S)); % for the boundary conditions
...
CallExact = @(S0) S0.*(1 - ncx2cdf(y,z,x(S0)))...
    - K.*exp(-r*T).*ncx2cdf(x(S0),z-2,y); 

figure()
hold on 
plot(s,DiffPrice,'k.') 
plot(s,CallExact(s),'b-')
hold off 

figure() 
hold on 
plot(s,CallExact(s)-DiffPrice.' )
hold off 