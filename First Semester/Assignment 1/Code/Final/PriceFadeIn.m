function value = PriceFadeIn(eta,sigma, K)
...
% Alpha for the control variates
cov_fg = cov(0.5*(fade_m + fade),0.5*(vanilla + vanilla_m));
alpha = -cov_fg(1,2)/cov_fg(2,2);

% Extract standard normal variates and compute Antithetic estimates
Z = Z0(1001:end,:);
Si = S0*exp(cumsum((r-0.5*sigma^2)*deltat + sigma*sqrt(deltat).*Z,2));
vanilla = exp(-r*T).*(max(eta.*(Si(:,end ) - K),0));
Si(:,end) = [];
...
% Final Value 
value = mean(0.5*(fade_m + fade) ...
    + alpha*(0.5*(vanilla + vanilla_m) - BSoption)); 
end 