function value = anti_strat_soln(sigma, K, eta, n)
% Fixed Input values
...
% Creating the antithetic estimate where each estimate centred about the
% mean of that particular strata

Unums_minus = 2*(index - 0.5)/d - Unums; 
Zminus = norminv(-Unums_minus); 

% The Brownian Bridge
Z = reshape(Z,[],1);
Zminus = reshape(Zminus,[],1);

Z1 = randn(n,N); % generating all at the same time

ST = S0*exp((r - 0.5*sigma^2)*T + sigma*sqrt(T).*Z);
STminus = S0*exp((r - 0.5*sigma^2)*T + sigma*sqrt(T).*Zminus);

W0 = zeros(n,1);
W100 = ((log(ST./S0) - (r - 0.5*sigma^2)*T)/sigma);
W100minus = ((log(STminus./S0) - (r - 0.5*sigma^2)*T)/sigma);

W = BrownianBridgeV(t,W0,W100,Z1); 
Wminus = BrownianBridgeV(t,W0,W100minus,Z1); 

Si = S0*exp((r - 0.5*sigma^2)*t(2:end-1) + sigma.*W);
Si_minus = S0*exp((r - 0.5*sigma^2)*t(2:end-1) + sigma.*Wminus);

vanilla = exp(-r*T).*(max(eta.*(ST - K),0)); 
vanilla_minus = exp(-r*T).*(max(eta.*(STminus - K),0)); 

fade = vanilla.*mean(Si > L & Si < U,2);
fade_minus  = vanilla_minus.*mean(Si_minus > L & Si_minus < U,2);
fade = (fade + fade_minus); 
